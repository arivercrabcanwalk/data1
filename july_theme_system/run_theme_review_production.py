...
class ProductionThemeReviewBT(ThemeReviewBT):
    ...
    @staticmethod
    def _stock_limit_price(prev_close, pct_magnitude, side):
        if not np.isfinite(prev_close) or not np.isfinite(pct_magnitude):
            return np.nan
        prev=Decimal(str(float(prev_close)))
        pct=Decimal(str(float(pct_magnitude)))
        mult=(Decimal('1')+pct) if side=='BUY' else (Decimal('1')-pct)
        return float((prev*mult).quantize(Decimal('0.01'),rounding=ROUND_HALF_UP))

    def can_trade_stock(self,st,code,side,raw,prev_close,bar=None):
        """Conservative execution feasibility using exact 0.01 stock price ticks.

        A bar at the price limit is treated as non-guaranteed execution because
        minute data cannot reconstruct queue priority. One tick away is allowed
        when actual minute volume is positive.
        """
        ...
        if sr is not None:
            for c in ['is_st','is_star_st','is_new_listing_initial']:
                if c in sr:
                    if pd.isna(sr[c]):
                        if side=='BUY': return False,f'unknown_{c}'
                    elif bool(sr[c]): return False,c
            if 'is_suspended' in sr and not pd.isna(sr['is_suspended']) and bool(sr['is_suspended']):
                return False,'suspended'
            if side=='BUY':
                p=abs_pct(sr.get('price_limit_up_pct',np.nan)); lim=self._stock_limit_price(prev_close,p,'BUY')
                if np.isfinite(lim) and raw>=lim-1e-9:return False,'locked_limit_up'
            else:
                p=abs_pct(sr.get('price_limit_down_pct',np.nan)); lim=self._stock_limit_price(prev_close,p,'SELL')
                if np.isfinite(lim) and raw<=lim+1e-9:return False,'locked_limit_down'
        if bar is not None and 'volume' in bar:
            if pd.isna(bar.volume) or float(bar.volume)<=0:return False,'zero_or_unknown_volume_fill_bar'
        return True,''
    ...
    def export_all(self,final,bench):
        ...
        gp=float(t.loc[t.pnl>0,'pnl'].sum()) if len(t) else 0.0; gl=float(-t.loc[t.pnl<0,'pnl'].sum()) if len(t) else 0.0; pf=gp/gl if gl>0 else (999.0 if gp>0 else 0.0)
        summary={'method_version':self.rules['version'],'data_scope':'GitHub 2026-07 minute1 + same-day status; web theme/core context only after prior close valid_from','cold_start':'2026-07-01 is observation-only because GitHub July-only scope has no 2026-06-30 prior close','start_equity':START,'final_equity_mark_to_market':final,'total_return':final/START-1,'max_drawdown':float(min(x['drawdown'] for x in self.eq)) if self.eq else 0.0,'benchmark_equal_weight_return_from_available_july_prev_closes':bench-1,'closed_trades':int(len(t)),'open_positions_month_end':int(len(self.pos)),'win_rate':float((t.net_return>0).mean()) if len(t) else 0.0,'profit_factor':pf,'total_closed_pnl':float(t.pnl.sum()) if len(t) else 0.0,'prediction_hit_rate':float(pr.prediction_hit.mean()) if len(pr) else np.nan,'missed_plan_universe_winners':int(len(mi)),'rule_threshold_changes_during_july':0,'learning_policy':'causal half-size/pause only; no threshold retuning unless >=8 closed samples and repeated mechanism','month_end_measurement':'sell at 2026-07-31 final minute only if exact tick/status/volume feasibility permits; otherwise mark to close'}
        (OUT/'summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2),encoding='utf-8')
        open_rows=[]
        for p in self.pos.values():
            mark=self.last_close.get(p['code'],p['last_mark']); open_rows.append({**p,'month_end_mark':mark,'unrealized_return':mark/p['entry_price']-1})
        pd.DataFrame(open_rows).to_csv(OUT/'open_positions.csv',index=False,encoding='utf-8-sig'); print(json.dumps(summary,ensure_ascii=False,indent=2))

    def run(self):
        bench=1.0; prev_eq=START; last_bundle=None; paths=self.paths()
        for path in paths:
            date=path.parent.name.split('=',1)[1]; pre_pause={m:s['pause_days'] for m,s in self.learn.items()}; new_trade_start=len(self.trades)
            df,daily,st,snaps=self.load_day(path); last_bundle=(date,df,daily,st,snaps); close_map=dict(zip(daily.code,daily.close))
            self.process_exits(date,df,daily,st,snaps); self.process_entries(date,df,daily,st,snaps); self.update_intraday_extremes(df,date,until=None)
            ...
            self.eq.append({'date':date,'equity':eq,'day_return':dayret,'drawdown':dd,'benchmark_cumulative':bench-1}); prev_eq=eq; self.decrement_pause_after_day(pre_pause); self.last_close=close_map

        date,df,daily,st,snaps=last_bundle
        for code in list(self.pos):
            z=df[df.code==code]
            if z.empty:continue
            b=z.iloc[-1].copy(); b['open']=b['close']; b['volume']=float(b.get('volume',0))
        ...
        if self.eq:
            prior=self.eq[-2]['equity'] if len(self.eq)>1 else START; self.eq[-1]['equity']=final; self.eq[-1]['day_return']=final/prior-1; peak=START
            for row in self.eq:
                peak=max(peak,row['equity']); row['drawdown']=row['equity']/peak-1
            self.daily[-1]['eod_equity']=final; self.daily[-1]['strategy_day_return']=final/prior-1; self.daily[-1]['drawdown']=self.eq[-1]['drawdown']; self.daily[-1]['positions_eod']=len(self.pos)
        self.export_all(final,bench)


if __name__=='__main__':ProductionThemeReviewBT().run()
