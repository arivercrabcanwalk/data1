...
# 6) Production buy path works with itertuples candidates and never uses dict-style row access.
pbt=ProductionThemeReviewBT(); pbt.cash=100000.0
...
# 7) Causal learning policy: no threshold changes; only future half-size/pause state.
...
# 8) Every non-cash playbook has explicit observation windows and next-minute entry windows.
...
# 9) Core universe cannot be generated by price ranking: it must come from prior web context tables.
...
# 10) Mode selection is deterministic by predeclared theme priority/role/mode order, never future PnL.
