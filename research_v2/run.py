...
import numpy as np
import pandas as pd
from core import (SOURCE_COMMIT, Market, Portfolio, audit_and_daily, configurations,
                  day_arrays, make_candidates, read_minute, save_json)
...
def main():
    ...
    if not eval_days or eval_days[0] < 60:
        raise ValueError('Study requires at least 60 prior trading observations')
    ...
    events=[]
    for p in ports:
        for r in p.opportunities:
            events.append(dict(variant=p.cfg.name,**r))
    ...
    hashes={}
    for name in ['core.py','run.py','test_core.py']:
    ...
    manifest=dict(status='completed', source_commit=SOURCE_COMMIT,
        ...
        runtime_seconds=round(time.monotonic()-start_clock,2),
        primary_model='MAIN_CLOSE',optimization='none; no performance-based parameter refit',
        scope='technical proxy; unadjusted price return; not author full industrial research system',
        ...
        limits=['ST updates after 2026-07-30 unavailable; last historical status carried forward',
                ...
                'retrospective supplied method documents; no performance guarantee'])
